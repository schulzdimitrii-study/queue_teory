...
class MM1K(BaseQueueModel):
    def __init__(
        self, lamb: float, mu: float, k: int, s: int, n: int, t: float
    ) -> None:
        ...
        if n < 0 or k < 0 or t < 0:
            raise ValueError("Parâmetros inválidos: n, k e t devem ser não negativos.")
    ...
    def __calculate_waiting_time_system_exceeding_t(self) -> float:
        return exp(-self.mu * self.t * (1 - self.rho))

    def __calculate_waiting_time_queue_exceeding_t(self) -> float:
        return self.rho * exp(-self.mu * self.t * (1 - self.rho))